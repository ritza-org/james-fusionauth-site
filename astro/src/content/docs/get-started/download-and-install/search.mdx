...
This page explains how to configure simple database search for FusionAuth, or install Elasticsearch or OpenSearch for more powerful features.

By default, FusionAuth provides a basic database search to find Users or Entities by simple fields like username or email address, including the ability to search by partial strings.
...
<Aside type="note">
FusionAuth supports Elasticsearch and OpenSearch with the same features. You can choose either brand. This guide uses "Elasticsearch" as a general term.
</Aside>

You can see which type of search your FA instance uses by browsing to <Breadcrumb>System -> About</Breadcrumb>.
...
## Should use database search or Elasticsearch?
...
Switching to Elasticsearch will require more RAM, more CPU (for continuous indexing even if you aren't currently searching), and disk space of about 1.5 to 2 times your user data in the database expanded into the search index. Elasticsearch synchronizes its index with the user database quickly, but gradually. So there might be small intervals when your search results don't represent the current users in the database.

However, you can switch your search provider type as much as you like. It's as simple as changing your Docker compose file, which is shown in this guide.
...
<Tabs>
	...
	To install on a Red Hat system, use the RPM bundle. Execute this command to install the FusionAuth Search RPM (replace `<version>` with
	...
	To install on a Debian system, use the DEB bundle. Execute this command to install the FusionAuth Search DEB (replace `<version>` with
 	the correct version number):
	...
	<TabItem label="Windows (Package)">

	Extract the `fusionauth-search` zip bundle anywhere on the file system. Remember where you extract
	...
	Once the zip bundle has been extracted, the directory structure should look similar to this:
	...
	1. Extract the `fusionauth-search` zip bundle anywhere on the file system. Remember where you extract
	 the file. This location will be referred to as `FUSIONAUTH_HOME`. We suggest extracting this file to a directory such as `\fusionauth` on Windows. Once the zip bundle has been extracted, the directory structure should look similar to this:
	...
	1. Run the following command to install FusionAuth as a Windows Service:
