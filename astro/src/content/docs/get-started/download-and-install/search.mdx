...
Switching from database search to Elasticsearch or OpenSearch reverses the process above. Here is an example [Docker Compose file that uses OpenSearch](https://github.com/ritza-co/fusionauth-example-docker-compose/blob/main/kickstart/docker-compose.yml). To use Elasticsearch instead, replace the `opensearchproject/opensearch` image with `docker.elastic.co/elasticsearch/elasticsearch` and update the environment variables accordingly.
...
In addition to the two settings above, you need to have an instance of Elasticsearch running. In this example, OpenSearch is running in Docker on the same machine as FusionAuth, but you can run Elasticsearch anywhere on the network. If you are not using Docker, refer to the [Elasticsearch installation docs](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch) or the [OpenSearch installation docs](https://docs.opensearch.org/latest/install-and-configure/install-opensearch/index/) for your platform.
