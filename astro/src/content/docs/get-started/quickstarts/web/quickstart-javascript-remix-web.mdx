---
title: Remix
description: Quickstart a Remix web application with FusionAuth.
---
...
This quickstart adds login to a Remix web app using a preconfigured, local version of FusionAuth in the following steps:

1. Run FusionAuth with Docker.
1. Create a new Remix application with React Router's scaffolding tool.
1. Add an Authenticator service and routes for login, logout, and the OAuth callback.
1. Add account and change pages that are only reachable when logged in.
...
1. Change into the new project and pin the dev server to port 3000. Open `mysite/vite.config.ts` and add a `server` block:
...
1. Add additional npm packages the site will use using the commands below.

   ```shell
   cd mysite
   npm install remix-auth remix-auth-oauth2 jwt-decode
   ```

   The two Remix authentication packages allow you to use FusionAuth without needing to know how OAuth works. The JWT package will provide your app with the user details returned from FusionAuth.

1. Create the `.env` file in your `mysite` folder and add the following to it (note that this is a different environment file to the one in the root folder used by Docker for FusionAuth). Vite's dev server automatically loads variables from this file, making them available to your server-side code via `process.env`.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/.env" lang="shell" />

### Preliminary Setup

Before you configure authentication, you need to change the default files created by the scaffolding tool.

1. **Delete the scaffolded `mysite/app/routes/home.tsx` file and the `mysite/app/welcome/` folder.** You'll add your own routes as you go, so these defaults aren't needed.

1. Replace the contents of the file `mysite/app/root.tsx` with the following code.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/root.tsx" lang="tsx" />
...
1. Routes are no longer discovered automatically by filename. Instead, `mysite/app/routes.ts` lists every route explicitly. Create it with the following content. You'll add to this file as you create each route below.
...
1. In this `services` folder, add a file called `session.server.ts` and copy in the following code (this guide follows the example from the [Remix Auth README](https://github.com/sergiodxa/remix-auth)).

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/services/session.server.ts" lang="ts" />
...
1. Create an authentication file in the `services` folder, `auth.server.ts`, that uses the session provider you just created.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/services/auth.server.ts" lang="ts" />
...
1. Create the homepage of your app at `mysite/app/routes/_index.tsx` with the following code:

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/routes/_index.tsx" lang="tsx" />
...
1. The link you created looks for a login route, so let's code it. Create the file `mysite/app/routes/login.tsx` and add the following code to it.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/routes/login.tsx" lang="tsx" />
...
1. Create the file `mysite/app/routes/logout.tsx` and add the following code to it.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/routes/logout.tsx" lang="tsx" />

   When a user browses to this route, their session cookie is destroyed and they're redirected to the home page.

1. The final route to create is `mysite/app/routes/auth.callback.tsx`.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/routes/auth.callback.tsx" lang="tsx" />
...
1. Add all the routes to `mysite/app/routes.ts`, replacing its contents with:
...
1. Copy over some CSS and an image from the example app.
...
1. Add the account page `mysite/app/routes/account.tsx` with the following code.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/routes/account.tsx" lang="tsx" />
...
1. Add the final page, `mysite/app/routes/change.tsx`, with the following code.

   <LocalCode src="fusionauth-quickstart-javascript-remix-web-main/complete-application/app/routes/change.tsx" lang="tsx" />
...
1. Add both pages to `mysite/app/routes.ts`:
...
1. Start the server from the root `mysite` directory.

   ```shell
   npm run dev
   ```

1. Browse to [the app](http://localhost:3000).

1. Log in using `richard@example.com` and `password`. The change page will allow you to enter a number. Log out and verify that you can't browse to the [account page](http://localhost:3000/account).
...
* Reading more about [Remix Auth](https://github.com/sergiodxa/remix-auth) and the [Remix-Auth-OAuth2 strategy](https://github.com/sergiodxa/remix-auth-oauth2) this guide is built on, or [Passport.js authentication concepts](https://www.passportjs.org/concepts/authentication/downloads/html/), which Remix Auth is modeled on.
