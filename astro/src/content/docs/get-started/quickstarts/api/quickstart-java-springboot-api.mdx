...
import SpringBootPreReqs from 'src/components/quickstarts/springboot-prereqs.mdx';
...
<Intro technology="Java Spring Boot"
...
<a href='https://start.spring.io/' target='_blank'>Go to the Spring Initializr site</a> and generate your own starter package, adding two dependencies:
...
If you choose different options, the configuration and code may be different — in particular, the `Group` and `Artifact` values determine the Java package (and therefore the folder path) your own source files will live in, covered in [Add Model Objects](#add-model-objects) below.

![The Spring Initializr form configured with Group io.fusionauth.quickstart, Artifact springapi, Name FusionAuthQuickstart, Spring Boot 4.0.7, Java 17, and the OAuth2 Resource Server and Spring Web dependencies added.](/img/quickstarts/quickstart-java-springboot-api/initializr-config.png)

Clicking "Generate" on the Spring Initializr site downloads a `springapi.zip` file to your downloads folder, containing the generated project in a top-level `springapi/` directory. Move that zip into `spring-api` and unzip it, then move its contents up a level so the project files sit directly in `spring-api` (matching the `cd spring-api` you did above) rather than in a nested `spring-api/springapi/` directory:
...
Open `src/main/resources/application.properties` and add the following two lines to the file:

<LocalCode src="quickstart-springboot-api/complete-application/src/main/resources/application.properties" lang="ini" />
...
We will need response objects for the API to return. One is the `PanicResponse`, which returns a message when a successful `POST` call is made to `/panic`. In the base package (`src/main/java/io/fusionauth/quickstart/springapi/`, derived from the `Group` and `Artifact` values you chose when generating the project) create a new class `PanicResponse.java`:

<LocalCode src="quickstart-springboot-api/complete-application/src/main/java/io/fusionauth/quickstart/springapi/PanicResponse.java" lang="java" />
...
<LocalCode src="quickstart-springboot-api/complete-application/src/main/java/io/fusionauth/quickstart/springapi/Change.java" lang="java" />

### Add Controllers

Next you need to add controllers to handle API calls. In the base package add a new class `MakeChangeController.java`.

<LocalCode src="quickstart-springboot-api/complete-application/src/main/java/io/fusionauth/quickstart/springapi/MakeChangeController.java" lang="java" />
...
<LocalCode src="quickstart-springboot-api/complete-application/src/main/java/io/fusionauth/quickstart/springapi/PanicController.java" lang="java" />
...
<LocalCode src="quickstart-springboot-api/complete-application/src/main/java/io/fusionauth/quickstart/springapi/CustomJwtAuthenticationConverter.java" lang="java" />
...
<LocalCode src="quickstart-springboot-api/complete-application/src/main/java/io/fusionauth/quickstart/springapi/SecurityConfiguration.java" lang="java" />
...
1. Copy the `token` from the response, which should look like this:
